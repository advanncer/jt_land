...
import {
  ChevronRight, ArrowLeft, Loader2, CheckCircle2, Sparkles, Users, Cake, Target, BarChart3, Heart, Frown, Clock, Coffee, Smartphone, UserCheck, TrendingUp, Calendar, Group, Trophy, Gift, Wand2, User, Mail, Globe2, Briefcase, BookOpen, MessageSquare, TrendingDown, Zap, Rocket, Shuffle, ThumbsUp, ThumbsDown, Check, FileText, Globe, Mic, Edit, Headphones, Award, Film, Shield, PlaySquare, HelpCircle, XCircle, LayoutGrid, CalendarDays, Monitor, HeartHandshake, Timer, Plane, Moon, Battery, BatteryLow, BatteryMedium, BatteryFull, Sunrise, Sun, Sunset
} from "lucide-react";
import { quizData } from "./data";

const icons: { [key: string]: React.FC<any> } = {
  Users, Cake, Target, BarChart3, Heart, Frown, Clock, Coffee, Smartphone, UserCheck, TrendingUp, Calendar, Group, Trophy, Gift, Wand2, User, Mail, Sparkles, Globe2, Briefcase, BookOpen, MessageSquare, TrendingDown, Zap, Rocket, Shuffle, ThumbsUp, ThumbsDown, Check, FileText, Globe, Mic, Edit, Headphones, Award, Film, Shield, PlaySquare, HelpCircle, XCircle, LayoutGrid, CalendarDays, Monitor, HeartHandshake, Timer, Plane, Moon, Battery, BatteryLow, BatteryMedium, BatteryFull, Sunrise, Sun, Sunset
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    dl.push({
      ...
      quiz_name: "eng-child-lp-js-child",
      step_number: step
    });
  }, [step]);
  ...
  const currentQuestionIndex = quizData.filter(
    (s) => s.step <= step && (s.type === "choice" || s.type === "testimonials_interstitial"),
  ).length;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const qaArray = Object.entries(answers).map(([step, answer]) => `Q${step}: ${answer}`);
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-slate-50 text-slate-900 font-sans flex flex-col items-center overflow-x-hidden selection:bg-orange-100 relative">
      ...
      {step > 1 && !["loader", "program_ready", "lead_name", "lead_contacts"].includes(currentStep?.type || "") && (
      ...
      <main className="flex-1 w-full max-w-lg flex flex-col p-4 justify-start overflow-y-auto custom-scrollbar relative z-10">
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full h-full flex flex-col pt-2">
            {IconComponent && !["hero", "loader", "program_ready", "lead_name", "lead_contacts"].includes(currentStep?.type || "") && (
              ...
              <div className="text-center flex flex-col items-center h-full">
                ...
                <div className="w-full mt-auto">
                  <div className="p-5 bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 mb-4">
                    ...
                    {currentStep.meta && <div className="flex items-center justify-center gap-1.5 mt-3 text-orange-600 font-bold text-[9px] uppercase tracking-widest bg-orange-50 w-fit mx-auto px-3 py-1 rounded-full border border-orange-100"><Sparkles size={10} /> <span>{currentStep.meta}</span></div>}
                  </div>
                  {currentStep.social_proof && <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest pb-2">{currentStep.social_proof}</p>}
                </div>
              </div>
            )}
            {["choice", "testimonials_interstitial"].includes(currentStep?.type || "") && (
              <div className="text-center flex flex-col h-full">
                ...
                {currentStep.type === "testimonials_interstitial" && currentStep.reviews && (
                    <div className="mb-6 flex-1 flex flex-col min-h-0">
                        ...
                        {currentStep.reviews.map((rev) => (
                          <div key={rev.name} className="bg-white p-5 rounded-3xl border border-slate-100 text-left shrink-0 w-[85%] max-w-[280px] shadow-sm snap-center relative flex flex-col h-full">
                            <div className="flex items-center gap-3 mb-3">
                              <img src={rev.photoUrl} className="w-10 h-10 rounded-full object-cover" alt={rev.name} />
                      ...
                      <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest flex items-center justify-center gap-2 mt-2"><ArrowLeft size={10} /> свайп відгуки <ChevronRight size={10} /></div>
                    </div>
                  )}
                  ...
                  {currentStep.options?.map((opt) => {
                    ...
                    return (
                      <button key={opt.value} onClick={() => handleChoice(opt.label)} className="w-full text-left p-3.5 rounded-2xl border-2 border-slate-100 bg-white hover:border-orange-500 active:bg-orange-50 transition-all font-bold text-sm flex items-center gap-4 group shadow-sm active:scale-[0.98]">
                        {OptIcon && <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-orange-100 group-hover:text-orange-500 transition-colors shrink-0"><OptIcon size={20} strokeWidth={2} /></div>}
                        <span className="flex-1 leading-snug">{opt.label}</span>
                      </button>
                    );
                  })}
                </div>
                {currentStep.type === "testimonials_interstitial" && <button onClick={() => setStep(step + 1)} className="w-full mt-2 py-4 bg-slate-900 text-white rounded-2xl font-black text-lg uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 active:scale-95">{currentStep.cta}</button>}
              </div>
            )}
            {currentStep?.type === "program_ready" && (
              <div className="bg-slate-900 text-white p-6 md:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col text-center flex-1 my-4">
                ...
                <div className="w-16 h-16 bg-orange-500/20 text-orange-500 rounded-full flex items-center justify-center mx-auto mb-6 relative z-10 shrink-0"><Sparkles size={32} strokeWidth={2.5} /></div>
                ...
                <ul className="space-y-3 mb-8 relative z-10 text-left mt-auto">
                  {currentStep.points?.map((p) => (
                    <li key={p} className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/5"><CheckCircle2 className="text-orange-500 shrink-0" size={20} /><span className="text-sm text-slate-200 font-bold leading-tight">{p}</span></li>
                  ))}
                </ul>
            ...
            {["lead_name", "lead_contacts"].includes(currentStep?.type || "") && (
              <div className="text-center flex flex-col h-full py-4">
                {currentStep.type === "lead_name" && <div className="w-16 h-16 bg-orange-100 text-orange-600 rounded-[1.25rem] flex items-center justify-center mx-auto mb-6 shrink-0 shadow-inner border border-orange-200/50"><User size={32} strokeWidth={2.5} /></div>}
                ...
                <div className="mt-auto w-full">
                  ...
                  {currentStep.type === "lead_name" ? (
                    <div className="space-y-4 w-full">
                      <input type="text" placeholder={currentStep.form?.name_placeholder || "Ваше ім'я"} className="w-full p-4 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-xl font-black text-slate-900 bg-white shadow-sm text-center" value={leadName} onChange={(e) => setLeadName(e.target.value)} />
                      <button onClick={() => leadName.trim().length >= 2 && setStep(step + 1)} disabled={leadName.trim().length < 2} className="w-full py-5 bg-orange-500 text-white rounded-2xl font-black text-lg shadow-lg shadow-orange-200 active:scale-95 transition-all disabled:opacity-30 uppercase tracking-widest">{currentStep.form?.cta || "Далі"}</button>
                    </div>
                  ) : (
                    <form onSubmit={handleSubmit} className="space-y-4 w-full">
                      <input type="email" placeholder={currentStep.form?.email_placeholder || "Ваш e-mail"} required className="w-full p-4 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-lg font-black text-slate-900 bg-white shadow-sm text-center" value={leadEmail} onChange={(e) => setLeadEmail(e.target.value)} />
                      <input type="tel" placeholder={currentStep.form?.phone_placeholder || "Твій номер телефону"} required className="w-full p-4 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-lg font-black text-slate-900 bg-white shadow-sm text-center" value={leadPhone} onChange={(e) => setLeadPhone(formatPhoneNumber(e.target.value))} maxLength={19} />
                      <button type="submit" disabled={leadPhone.replace(/\D/g, "").length !== 12} className="w-full py-5 bg-orange-500 text-white rounded-2xl font-black text-lg shadow-lg shadow-orange-200 active:scale-95 transition-all disabled:opacity-30 uppercase tracking-widest">{currentStep.form?.cta || "Відправити"}</button>
            ...
            {currentStep?.type === "loader" && (
              <div className="text-center py-8 px-4 flex flex-col justify-center flex-1 h-full">
                ...
                <div className="w-full max-w-xs mx-auto bg-slate-100 h-2 rounded-full overflow-hidden mb-6"><div className="h-full bg-orange-500" style={{ width: `${loaderProgress}%` }} /></div>
                ...
                <div className="grid gap-3 max-w-[280px] mx-auto text-left w-full mt-auto mb-10">
                  {currentStep.points?.map((p, i) => (
                    <div key={p} className={`flex items-center gap-4 text-[10px] font-black uppercase tracking-wider transition-opacity duration-500 ${loaderProgress > i * 25 ? "text-slate-900" : "text-slate-200"}`}><div className={`w-2 h-2 rounded-full shrink-0 ${loaderProgress > i * 25 ? "bg-orange-500" : "bg-slate-200"}`} />{p}</div>
                  ))}
                </div>
              </div>
            )}
      ...
      <style>{`body { overflow-x: hidden; width: 100%; position: relative; background: #f8fafc; } input::placeholder { color: #cbd5e1; font-weight: 700; text-transform: uppercase; font-size: 12px; letter-spacing: 0.05em; text-align: center; } .custom-scrollbar::-webkit-scrollbar { height: 4px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #f97316; border-radius: 10px; }`}</style>
    </div>
  );
}
