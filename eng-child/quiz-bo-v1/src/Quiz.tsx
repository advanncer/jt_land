...
const formatPhoneNumber = (value: string) => {
    ...
    let numbers = input;
    if (numbers.startsWith("380")) { /* do nothing */ }
...
const Quiz: React.FC<QuizProps> = ({ step, onNextStep }) => {
  ...
  const renderStart = () => (
    <>
      <h1 className="text-2xl sm:text-3xl font-bold text-white mb-4 leading-tight text-center">
        Дитина заговорить англійською вже через кілька занять: <span className="text-brand-orange">безкоштовний пробний урок + гарантія результату</span>
      </h1>
      <p className="text-gray-300 font-semibold mb-3 text-center">Пройдіть короткий тест — і отримайте:</p>
      <ul className="text-gray-400 space-y-3 mb-6 mx-auto max-w-sm">
        <li className="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/10"><span className="text-2xl">👨‍🏫</span><span>Ідеального репетитора під ваші потреби</span></li>
        <li className="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/10"><span className="text-2xl">🎁</span><span>Корисний PDF подарунок для дитини</span></li>
      </ul>
      <div className="w-full h-px bg-white/10 my-6"></div>
      <h2 className="text-xl font-bold text-white mb-4 text-center">{currentStepData.question}</h2>
  ...
  const renderChoice = () => (
    <>
      <h2 className="text-xl sm:text-2xl font-bold text-white mb-2 text-center">{currentStepData.question}</h2>
      {currentStepData.desc && <p className="text-gray-400 text-center mb-6">{currentStepData.desc}</p>}
  ...
  const renderBanner1 = () => (
    <>
      <h2 className="text-2xl font-bold text-white mb-6 text-center">Чому обирають JustSchool?</h2>
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-white/5 border border-white/10 rounded-xl p-4"><div className="text-3xl mb-2">🎁</div><div className="font-bold text-sm text-white mb-1">Перший урок — безкоштовно</div><div className="text-xs text-gray-400">Бачите прогрес дитини, перш ніж платити хоч копійку</div></div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-4"><div className="text-3xl mb-2">🎮</div><div className="font-bold text-sm text-white mb-1">Навчання через гру</div><div className="text-xs text-gray-400">Авторські інтерактивні квести замість нудних підручників</div></div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-4"><div className="text-3xl mb-2">🎯</div><div className="font-bold text-sm text-white mb-1">Програма під дитину</div><div className="text-xs text-gray-400">Індивідуально або в групі, 6–17 років</div></div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-4"><div className="text-3xl mb-2">🏆</div><div className="font-bold text-sm text-white mb-1">Трамплін у топові виші</div><div className="text-xs text-gray-400">Підготовка до НМТ/ЗНО та вступ за кордон</div></div>
      </div>
    </>
  );
  ...
  const renderStats = () => (
    <>
      <h2 className="text-2xl font-bold text-white mb-6 text-center">JustSchool у цифрах</h2>
      <div className="flex flex-col gap-3 mb-6">
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center"><div className="text-3xl font-extrabold text-brand-orange mb-1">15 000+</div><div className="text-sm text-gray-300">випускників школи за весь час роботи</div></div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center"><div className="text-3xl font-extrabold text-brand-orange mb-1">4 міс</div><div className="text-sm text-gray-300">середній час підняття рівня (A2→B1)</div></div>
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center"><div className="text-3xl font-extrabold text-brand-orange mb-1">187</div><div className="text-sm text-gray-300">середній бал з англійської на НМТ/ЗНО</div></div>
      </div>
    </>
  );
  ...
  const renderCompare = () => (
    <>
      <h2 className="text-2xl font-bold text-white mb-6 text-center">Чому JustSchool — інакше?</h2>
      <div className="flex flex-col gap-4 mb-6">
        {[
          ...
          <div key={i} className="bg-white/5 border border-brand-orange/30 rounded-xl p-4 relative overflow-hidden">
             <div className="absolute top-0 left-0 w-1 h-full bg-brand-orange"></div>
             <div className="text-xs font-bold text-brand-orange uppercase mb-1">JustSchool ✓</div>
             <div className="text-white font-semibold mb-3">{row[1]}</div>
             <div className="text-xs text-gray-500 uppercase mb-1">Інші школи</div>
             <div className="text-gray-500 line-through text-sm">{row[0]}</div>
          </div>
        ))}
      </div>
    </>
  ...
  const renderText = () => (
    <>
      <h2 className="text-2xl font-bold text-white mb-2 text-center">{currentStepData.question}</h2>
  ...
  if (currentStepData.type === 'banner1' || currentStepData.type === 'stats' || currentStepData.type === 'compare') {
    showCta = true;
  } else if (currentStepData.type === 'text') {
    showCta = true;
    ...
    ctaText = currentStepData.cta || "Отримати безплатну консультацію";
  }
