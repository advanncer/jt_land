...
const GOOGLE_SHEETS_WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbx1VN8FZ931HkEbAEHo3U6KlxcRAMbPG8vBjcQX-sw5NmgcEiymbXUlQFpNVM9z0q1Ijw/exec";
...
const App: React.FC = () => {
  ...
  const submitQuizData = async (finalAnswers: Record<number, string[]>) => {
    ...
    const formStepIndex = 12; // Крок з реєстрацією
      ...
      return (
        <div className="min-h-screen w-full bg-black flex justify-center relative overflow-hidden font-sans selection:bg-brand-orange/30">
            ...
            <div className="w-full max-w-[480px] bg-black/80 relative z-10 min-h-screen shadow-2xl flex flex-col items-center justify-center p-6 text-center border-x border-white/5 animate-fade-in-up">
                ...
                <p className="text-gray-300 text-lg mb-8 max-w-sm">Оберіть мессенджер, щоб завершити реєстрацію</p>
                <a
                ...
                <div className="mt-8 p-4 bg-white/5 border border-white/10 rounded-xl max-w-xs">
                  <p className="text-gray-400 text-sm">На email відправлено доступ до платформи, де можна пройти кілька вправ без викладача.</p>
                </div>
            </div>
        </div>
      );
      ...
      return (
        <div className="min-h-screen w-full bg-black flex justify-center relative overflow-hidden font-sans selection:bg-brand-orange/30">
            ...
            <div className="w-full max-w-[480px] bg-black/80 relative z-10 min-h-screen shadow-2xl flex flex-col items-center justify-center p-6 text-center border-x border-white/5 animate-fade-in-up">
                ...
                <h1 className="text-3xl font-bold text-white mb-4">Успішно!</h1>
                <p className="text-gray-300 text-lg mb-8 max-w-sm">
                  Ваша заявка прийнята. PDF-словник вже на вашій пошті. Найближчим часом ми відкриємо вам доступ до платформи.
                </p>
            </div>
        </div>
      );
