...
import { ChevronRight, ArrowLeft, Loader2, CheckCircle2, Sparkles, Users, Cake, Target, BarChart3, Heart, Frown, Clock, Coffee, Smartphone, UserCheck, TrendingUp, Calendar, Group, Trophy, Gift, Wand2, User, Mail, Globe2, Briefcase, BookOpen, MessageCircle, Home, Sunrise, Sun, Sunset, Moon, Battery, BatteryLow, BatteryMedium, BatteryFull, Smile, Book, MessageSquare, TrendingDown, Zap, Rocket, Shuffle, ThumbsUp, ThumbsDown, Check, FileText, Globe, Mic, Edit, Headphones, Award, Film, Shield, PlaySquare, HelpCircle, XCircle, LayoutGrid, CalendarDays, Monitor, HeartHandshake, Timer, Plane } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col items-center overflow-x-hidden selection:bg-orange-100">
      ...
      <main className="flex-1 w-full max-w-lg flex flex-col px-5 py-8 justify-start">
          ...
          <motion.div key={step} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full">
            ...
            {currentStep?.type === 'hero' && (
              <div className="text-center flex flex-col items-center">
                <h1 className="text-3xl md:text-5xl font-black mb-6 leading-tight tracking-tight">{currentStep.question}</h1>
                
                <div className="w-full grid gap-3 text-left mb-8">
                    <div className="flex items-start gap-3 bg-white border border-slate-100 p-4 rounded-2xl shadow-sm">
                        <MessageCircle size={20} className="text-orange-500 mt-0.5 shrink-0"/> 
                        <span className="text-sm font-bold text-slate-700 leading-snug">Долай мовний бар'єр вже за 1 місяць</span>
                    </div>
                    <div className="flex items-start gap-3 bg-white border border-slate-100 p-4 rounded-2xl shadow-sm">
                        <Smartphone size={20} className="text-orange-500 mt-0.5 shrink-0"/> 
                        <span className="text-sm font-bold text-slate-700 leading-snug">Інтерактивна платформа під твій темп</span>
                    </div>
                    <div className="flex items-start gap-3 bg-white border border-slate-100 p-4 rounded-2xl shadow-sm">
                        <Target size={20} className="text-orange-500 mt-0.5 shrink-0"/> 
                        <span className="text-sm font-bold text-slate-700 leading-snug">Персональний план навчання</span>
                    </div>
                </div>

                <div className="w-full p-6 bg-slate-100/50 rounded-3xl border border-slate-100">
                ...
                <p className="mt-6 text-[11px] font-black text-slate-400 uppercase tracking-widest">{currentStep.social_proof}</p>
              </div>
            )}
            ...
            {(currentStep?.type === 'choice' || currentStep?.type === 'testimonials_interstitial') && (
             ...
             {currentStep?.type === 'program_ready' && (
                <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col text-center">
            ...
            {(currentStep?.type === 'lead_name' || currentStep?.type === 'lead_contacts') && (
                <div className="text-center pt-4">
                    ...
                    <h2 className="text-2xl font-black mb-4 text-slate-900 leading-tight tracking-tight">{currentStep.title}</h2>
                    <p className="text-sm text-slate-500 mb-8 font-bold leading-relaxed max-w-sm mx-auto">{currentStep.subtitle}</p>
                    ...
                    {currentStep.type === 'lead_name' ? (
                       <div className="space-y-4">
                         <input type="text" placeholder={quizData.find(s=>s.type==='lead_name')?.form?.name_placeholder || "Ваше ім'я"} className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-lg font-black text-slate-900 bg-white shadow-sm" value={leadName} onChange={e => setLeadName(e.target.value)} />
                       ...
                       <form onSubmit={handleSubmit} className="space-y-4">
                         <input type="tel" placeholder="+380 (XX) XXX-XX-XX" required className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-lg font-black text-slate-900 bg-white shadow-sm" value={leadPhone} onChange={e => setLeadPhone(formatPhoneNumber(e.target.value))} maxLength={19} />
                         <input type="email" placeholder="Ваш e-mail" required className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-lg font-black text-slate-900 bg-white shadow-sm" value={leadEmail} onChange={e => setLeadEmail(e.target.value)} />
             ...
             {currentStep?.type === 'loader' && (
                <div className="text-center py-12 flex flex-col justify-center flex-1">
                    ...
                    <h2 className="text-2xl font-black mb-10 text-slate-900 leading-tight tracking-tight">{currentStep.title}</h2>
                    ...
                    <div className="grid gap-3 max-w-xs mx-auto text-left pl-4">
                        {currentStep.points?.map((p, i) => (
                            <div key={p} className={`flex items-center gap-4 text-xs font-black uppercase tracking-wider transition-opacity duration-500 ${loaderProgress > (i * 25) ? 'text-slate-800' : 'text-slate-300'}`}>
                                <div className={`w-2 h-2 rounded-full shrink-0 ${loaderProgress > (i * 25) ? 'bg-orange-500' : 'bg-slate-300'}`} />
                                {p}
                            </div>
                        ))}
                    </div>
