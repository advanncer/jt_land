...
export default function App() {
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 text-slate-900 font-sans flex flex-col items-center overflow-x-hidden selection:bg-orange-100">
      ...
      <main className="flex-1 w-full max-w-lg flex flex-col p-4 md:p-6 justify-start">
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full pt-4 md:pt-8 flex flex-col h-full min-h-[calc(100vh-100px)]">
            ...
            {currentStep?.type === 'hero' && (
              <div className="text-center flex flex-col items-center flex-1 justify-center">
                <h1 className="text-4xl md:text-5xl font-black mb-6 leading-tight tracking-tight">{currentStep.question || currentStep.title}</h1>
                
                <p className="text-base text-slate-600 mb-8 leading-relaxed font-medium">{currentStep.subtext || currentStep.subtitle}</p>
                <p className="text-[10px] font-black text-slate-400 uppercase mb-8 tracking-widest bg-slate-200 px-4 py-2 rounded-full inline-block">{currentStep.social_proof}</p>
            ...
            {(currentStep?.type === 'choice' || currentStep?.type === 'testimonials_interstitial') && (
              <div className="text-center flex flex-col flex-1 pb-4">
                <h2 className="text-2xl md:text-3xl font-black mb-3 leading-tight">{currentStep.question || currentStep.title}</h2>
                {currentStep.subtext && <p className="text-sm md:text-base text-slate-500 mb-6 font-medium leading-relaxed max-w-sm mx-auto">{currentStep.subtext || currentStep.subtitle}</p>}
                
                {currentStep.type === 'testimonials_interstitial' && currentStep.reviews && (
                  <div className="mb-6 mt-auto">
                      <div className="flex overflow-x-auto gap-4 pb-4 custom-scrollbar -mx-5 px-5 snap-x snap-mandatory">
                        {currentStep.reviews.map(rev => (
                           <div key={rev.name} className="bg-white p-4 rounded-3xl border border-slate-100 text-left shrink-0 w-[85%] shadow-sm snap-center relative overflow-hidden">
                ...
                <div className={`grid gap-2 w-full ${currentStep.type === 'choice' ? 'mt-auto' : ''}`}>
                  {currentStep.options?.map(opt => {
                    ...
                    return (
                        <button key={opt.value} onClick={() => handleChoice(opt.label)} className="w-full text-left p-4 rounded-2xl border-2 border-slate-100 bg-white hover:border-orange-500 active:bg-orange-50 transition-all font-bold text-sm md:text-base flex justify-start items-center gap-4 group shadow-sm">
                            {OptIcon && (
                ...
                {currentStep.type === 'testimonials_interstitial' && (
                     <button onClick={() => setStep(step + 1)} className="w-full mt-4 py-4 bg-slate-900 text-white rounded-2xl font-black text-lg uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 active:scale-95">{currentStep.cta}</button>
                )}
              </div>
            )}

             {currentStep?.type === 'program_ready' && (
                <div className="bg-slate-900 text-white p-8 md:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col text-center flex-1 justify-center mt-6">
                   ...
                   <h2 className="text-2xl font-black mb-4 leading-tight relative z-10">{currentStep.question || currentStep.title}</h2>
                   <p className="text-slate-400 text-sm mb-8 relative z-10 font-bold leading-relaxed">{currentStep.subtext || currentStep.subtitle}</p>
                   
                   <ul className="space-y-3 mb-10 relative z-10 text-left mt-auto">
                   ...
                   <button onClick={() => setStep(step + 1)} className="w-full py-5 bg-orange-500 text-white rounded-2xl font-black text-lg uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg shadow-orange-900/40 active:scale-95 relative z-10">{currentStep.cta}</button>
                </div>
            )}

            {(currentStep?.type === 'lead_name' || currentStep?.type === 'lead_contacts') && (
                <div className="text-center pt-4 flex flex-col flex-1 pb-6 justify-center mt-10">
                    <div className="w-20 h-20 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center mx-auto mb-6 shrink-0 shadow-lg shadow-orange-200/50">
                    ...
                    <div className="mt-auto">
                        ...
                        {currentStep.type === 'lead_name' ? (
                           <div className="space-y-4">
                             <input type="text" placeholder="Ваше ім'я" className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-xl font-black text-slate-900 bg-slate-50 text-center" value={leadName} onChange={e => setLeadName(e.target.value)} />
                           ...
                           <form onSubmit={handleSubmit} className="space-y-4">
                             <input type="tel" placeholder="+380 (XX) XXX-XX-XX" required className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-lg font-black text-slate-900 bg-slate-50 text-center" value={leadPhone} onChange={e => setLeadPhone(formatPhoneNumber(e.target.value))} maxLength={19} />
                             <input type="email" placeholder="Ваш e-mail" required className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-lg font-black text-slate-900 bg-slate-50 text-center" value={leadEmail} onChange={e => setLeadEmail(e.target.value)} />
             ...
             {currentStep?.type === 'loader' && (
                <div className="text-center py-8 px-4 flex flex-col justify-center flex-1">
                    <div className="w-20 h-20 bg-orange-100 text-orange-600 rounded-3xl flex items-center justify-center mx-auto mb-8"><Loader2 className="animate-spin" size={40}/></div>
