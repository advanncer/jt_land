...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col items-center overflow-x-hidden selection:bg-orange-100">
      ...
      <main className="flex-1 w-full max-w-lg flex flex-col p-4 md:p-6 justify-start min-h-[calc(100vh-60px)] relative">
          ...
          <motion.div key={step} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full pt-4 md:pt-8 flex flex-col h-full">
            ...
            {currentStep?.type === 'hero' && (
              <div className="text-center flex flex-col items-center flex-1 justify-center">
                <h1 className="text-4xl md:text-5xl font-black mb-6 leading-tight tracking-tight">{currentStep.title}</h1>
            ...
            {(currentStep?.type === 'lead_name' || currentStep?.type === 'lead_contacts') && (
                <div className="text-center flex flex-col flex-1 pb-6 justify-center">
                    ...
                    <div className="mt-auto">
                        ...
                        {currentStep.type === 'lead_name' ? (
                           <div className="space-y-4">
                             <input type="text" placeholder="Ваше ім'я" className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-xl font-black text-slate-900 bg-white shadow-sm" value={leadName} onChange={e => setLeadName(e.target.value)} />
                           ...
                           <form onSubmit={handleSubmit} className="space-y-4">
                             <input type="tel" placeholder="+380 (XX) XXX-XX-XX" required className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-xl font-black text-slate-900 bg-white shadow-sm" value={leadPhone} onChange={e => setLeadPhone(formatPhoneNumber(e.target.value))} maxLength={19} />
                             <input type="email" placeholder="Ваш e-mail" required className="w-full p-5 rounded-2xl border-2 border-slate-100 outline-none focus:border-orange-500 transition-all text-xl font-black text-slate-900 bg-white shadow-sm" value={leadEmail} onChange={e => setLeadEmail(e.target.value)} />
             ...
             {currentStep?.type === 'loader' && (
                <div className="text-center py-8 px-4 flex flex-col justify-center flex-1">
                    ...
                    <h2 className="text-2xl font-black mb-10 text-slate-900 leading-tight uppercase tracking-tight">{currentStep.title}</h2>
