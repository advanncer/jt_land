        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="lead" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col items-center justify-center py-10 w-full max-w-md mx-auto">
              <div className="glass p-8 md:p-12 rounded-[3rem] border border-white/10 text-center w-full bg-black/50 backdrop-blur-xl">
                ...
                <form onSubmit={handleLeadSubmit} className="space-y-4">
                  <input type="text" placeholder="Ім'я" required className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 outline-none focus:border-orange-500" value={leadName} onChange={e => setLeadName(e.target.value)} />
                  <input type="tel" placeholder="+380 (XX) XXX-XX-XX" required className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 outline-none focus:border-orange-500" value={leadPhone} onChange={e => setLeadPhone(formatPhoneNumber(e.target.value))} />
                  <input type="email" placeholder="Email" required className="w-full p-4 rounded-2xl bg-white/5 border border-white/10 outline-none focus:border-orange-500" value={leadEmail} onChange={e => setLeadEmail(e.target.value)} />
          ...
          {step === 'result' && result && (
            <div className="w-full py-10">
              ...
              <div className="glass p-6 md:p-12 rounded-[3rem] border border-white/10 mb-12 bg-white/5 backdrop-blur-3xl">
                <div className="grid grid-cols-2 gap-4 mb-10">
                  <div className="bg-white/5 p-4 rounded-2xl flex items-center gap-3"><Star className="text-purple-500"/> <div><div className="text-[10px] opacity-40 uppercase">Управитель</div><div className="font-bold">{selectedZodiac && ZODIAC_SIGNS.find(z=>z.id===selectedZodiac)?.ruler}</div></div></div>
                  <div className="bg-white/5 p-4 rounded-2xl flex items-center gap-3"><Flame className="text-orange-500"/> <div><div className="text-[10px] opacity-40 uppercase">Стихія</div><div className="font-bold">{selectedZodiac && ZODIAC_SIGNS.find(z=>z.id===selectedZodiac)?.element}</div></div></div>
                </div>
              ...
              <div className="flex flex-col items-center gap-12 mb-20">
                ...
                <div className="w-full max-w-md space-y-4 px-4">
                  <div className="grid grid-cols-3 gap-3">
                    <button onClick={()=>shareToPlatform('telegram')} className="p-5 glass rounded-2xl flex flex-col items-center gap-2 border border-white/5 active:bg-blue-500/20"><Send className="text-[#229ED9]"/><span className="text-[10px] font-bold opacity-50">TG</span></button>
                    <button onClick={()=>shareToPlatform('instagram')} className="p-5 glass rounded-2xl flex flex-col items-center gap-2 border border-white/5 active:bg-pink-500/20"><Instagram className="text-[#E4405F]"/><span className="text-[10px] font-bold opacity-50">IG</span></button>
                    <button onClick={()=>shareToPlatform('tiktok')} className="p-5 glass rounded-2xl flex flex-col items-center gap-2 border border-white/5 active:bg-white/20"><TikTokIcon className="text-white"/><span className="text-[10px] font-bold opacity-50">TIKTOK</span></button>
                  </div>
