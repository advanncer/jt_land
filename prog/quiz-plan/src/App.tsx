...
// Steps shown in the progress counter (choice + multi)
const PROGRESS_STEPS = [2, 3, 4, 6];
                ...
                {s.projects.map(proj => (
                  <div key={proj.id} className="bg-white p-2 rounded-2xl border border-violet-100 shadow-sm flex flex-col">
                    <div className="w-full relative rounded-xl overflow-hidden aspect-video bg-black">
                      <iframe 
                        src={`https://www.youtube.com/embed/${proj.id}?autoplay=1&mute=1&loop=1&playlist=${proj.id}&controls=0&modestbranding=1&playsinline=1&disablekb=1&start=10`}
          ...
          <div className="w-full">
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-2xl p-4 flex items-center gap-3 mb-6">
              ...
              <p className="text-orange-700 text-xs font-bold leading-tight">
                Безкоштовне пробне заняття з викладачем у подарунок! Отримай персональну програму навчання з програмування.
              </p>
            </div>
